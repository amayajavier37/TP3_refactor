...
public class PersonaRepository {
    ...
    /**
     * Busca por nombre a part
     */
    public Optional<List<Persona>> buscarPorNombre(String nombreOParte) {
        return jdbi.withHandle(handle -> {
            var rs = handle.select("select nombre, apellido from persona where nombre like ?").bind(0, "%" + nombreOParte + "%").mapToMap(String.class).list();
    ...
    public Optional<Persona> buscarId(Long id) {
        return jdbi.withHandle(handle -> {
            ...
            var rs = handle.select("select nombre, apellido from persona where id_persona = ?").bind(0, id).mapToMap(String.class).list();
